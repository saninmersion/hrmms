...
use App\Domain\Monitorings\Exports\SupervisorMonitoringExport;
...
/**
 * Class ExportSupervisorMonitoring
 * @package App\Domain\Monitorings\Actions
 */
class ExportSupervisorMonitoring implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(ZipService $zipService): void
    {
        ...
        $filename    = sprintf("supervisor_monitoring_%s.csv", now());
        $zipFilename = sprintf("supervisor_monitoring_%s.zip", now());
        ...
        try {
            (new SupervisorMonitoringExport())->export($tempPath);
