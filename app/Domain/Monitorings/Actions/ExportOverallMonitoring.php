...
use App\Domain\Monitorings\Exports\OverallMonitoringExport;
...
/**
 * Class ExportOverallMonitoring
 * @package App\Domain\Monitorings\Actions
 */
class ExportOverallMonitoring implements ShouldQueue
{
    use Dispatchable;
    ...
    public function handle(ZipService $zipService): void
    {
        ...
        $filename      = sprintf("overall_monitoring_%s.csv", now());
        $zipFilename   = sprintf("overall_monitoring_%s.zip", now());
        ...
        try {
            (new OverallMonitoringExport())->export($tempPath);
