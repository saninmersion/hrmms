...
use App\Domain\Applicants\Exports\ApplicationCorrectionNeededListExport;
...
/**
 * Class ExportApplicationCorrectionNeededList
...
class ExportApplicationCorrectionNeededList
{
    ...
    public function export(): void
    {
        ...
        $filename    = sprintf("correction_needed_applications.csv");
        $zipFilename = sprintf("correction_needed_applications.zip");
        $tempPath    = storage_path("exports/{$filename}");
        $path        = sprintf("%s/%s", General::PATH_EXPORTS, $zipFilename);

        try {
            (new ApplicationCorrectionNeededListExport())->export($tempPath);
