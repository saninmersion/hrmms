...
class ShortListedApplicantDetailTransformer extends TransformerAbstract
{
    ...
    public function transform(ApplicationListView $applicantView): array
    {
        ...
        return [
            ...
            'location_first'  => [
                ...
                'ward'         => $applicantView->first_priority_ward,
            ],
            'location_second' => [
                'district'     => $applicantView->second_priority_district,
                'municipality' => $applicantView->second_priority_municipality,
                'ward'         => $applicantView->second_priority_ward,
            ],
    ...
    protected function getPersonal(ApplicationListView $applicantView): array
    {
        return [
            ...
            'ethnicity'               => $applicantView->ethnicity ?? '',
            'ethnicity_formatted'     => $applicantView->ethnicity_formatted ?? '',
            'mother_tongue'           => $applicantView->mother_tongue ?? '',
            'mother_tongue_formatted' => $applicantView->mother_tongue_formatted ?? '',
            'disability'              => $applicantView->disability_formatted,
        ...
        return [
            ...
            'education'                      => [
                'supervisor' => [
                    ...
                    'percentage'          => $applicantView->percentage_supervisor ?? null,
                    'grade'               => $applicantView->grade_supervisor ?? null,
                    'major_subject'       => $applicantView->major_subject_supervisor ?? null,
                    'major_subject_other' => $applicantView->major_subject_others_supervisor ?? null,
                ],
                'enumerator' => [
                    'files'               => [],
                    'grading_system'      => data_get($applicantView, 'grading_system', ApplicationConstants::GRADING_SYSTEM_GRADE),
                    'percentage'          => $applicantView->percentage_enumerator ?? null,
                    'grade'               => $applicantView->grade_enumerator ?? null,
                ...
                'extra'      => [
                    'major_subject'       => $applicantView->major_subject_extra ?? null,
                    'major_subject_other' => $applicantView->major_subject_others_extra ?? null,
                ],
            ],
            'has_training'                   => (bool) ($applicantView->has_training ?? false),
            'training_documents'             => [],
            'training'                       => [
                'institute' => $applicantView->training_institute ?? '',
                'period'    => $applicantView->training_period_in_days,
                'files'     => [],
            ],
            'has_experience'                 => (bool) ($applicantView->has_experience ?? false),
            'experience_documents'           => [],
            'experience'                     => [
                'organization' => $applicantView->experience_organization ?? '',
                'period_day'   => $applicantView->experience_period_day ?? '',
                'period_month' => $applicantView->experience_period_month ?? '',
                'files'        => [],
            ],
        ];
    }
