...
class ApplicationCorrectionNeededListExport
{
    ...
    public function mapData(ApplicationListView $application): array
    {
        ...
        return [
            ...
            'Name (English) (Modified)'     => $applicationVerification->modified->name_in_english ?? '',
            'Name (Devnagari)'              => $application->name_in_nepali_formatted,
            'Name (Devnagari) (Modified)'   => $applicationVerification->modified->name_in_nepali ?? '',
            'Gender'                        => $application->gender_formatted,
            'Gender (Modified)'             => $applicationVerification->modified->gender ?? '',
            'Date of Birth (BS)'            => $application->dob_bs,
            'Date of Birth (BS) (Modified)' => $applicationVerification->modified->dob_bs ?? '',

            'Citizenship Number'                      => $application->citizenship_number,
            'Citizenship Number (Modified)'           => $applicationVerification->modified->citizenship_number ?? '',
            ...
            'Citizenship Issued Date (BS) (Modified)' => $applicationVerification->modified->citizenship_issued_date_bs ?? '',

            'Degree Document Provided (Supervisor)'            => $application->has_degree_for_supervisor_formatted,
            'Degree Document Provided (Supervisor) (Modified)' => $applicationVerification->modified->has_degree_for_supervisor ?? '',
            'Major subject (Supervisor)'                       => $application->major_subject_supervisor_formatted,
            'Major subject (Supervisor) (Modified)'            => $applicationVerification->modified->major_subject_supervisor ?? '',
            'Major subject (Supervisor, if others)'            => $application->major_subject_others_supervisor,
            'Major subject (Supervisor, if others) (Modified)' => $applicationVerification->modified->major_subject_others_supervisor ?? '',
            'CGPA (Supervisor)'                                => $application->grade_supervisor,
            'CGPA (Supervisor) (Modified)'                     => $applicationVerification->modified->grade_supervisor ?? '',
            'Percentage (Supervisor)'                          => $application->percentage_supervisor,
            'Percentage (Supervisor) (Modified)'               => $applicationVerification->modified->percentage_supervisor ?? '',

            'Degree Document Provided (Enumerator)'            => $application->has_degree_for_enumerator_formatted,
            'Degree Document Provided (Enumerator) (Modified)' => $applicationVerification->modified->has_degree_for_enumerator ?? '',
            'CGPA (Enumerator)'                                => $application->grade_enumerator,
            'CGPA (Enumerator) (Modified)'                     => $applicationVerification->modified->grade_enumerator ?? '',
            'Percentage (Enumerator)'                          => $application->percentage_enumerator,
            'Percentage (Enumerator) (Modified)'               => $applicationVerification->modified->percentage_enumerator ?? '',

            'Higher education Document Provided'                   => $application->has_higher_degree_formatted,
            'Higher education Document Provided (Modified)'        => $applicationVerification->modified->has_higher_degree ?? '',
            'Higher education Main subject'                        => $application->major_subject_higher_formatted,
            'Higher education Main subject (Modified)'             => $applicationVerification->modified->major_subject_higher ?? '',
            'Higher education Main subject (If others)'            => $application->major_subject_others_extra,
            'Higher education Main subject (If others) (Modified)' => $applicationVerification->modified->major_subject_others_extra ?? '',

            'Training Taken'                                => $application->has_training_formatted,
            'Training Taken (Modified)'                     => $applicationVerification->modified->has_training ?? '',
            'Training Taken (Document Provided)'            => $application->has_training_documents_formatted,
            'Training Taken (Document Provided) (Modified)' => $applicationVerification->modified->has_training_documents ?? '',
            'Training Institution'                          => $application->training_institute,
            'Training Institution (Modified)'               => $applicationVerification->modified->training_institute ?? '',
            'Training Days'                                 => $application->training_period_in_days,
            'Training Days (Modified)'                      => $applicationVerification->modified->training_period_in_days ?? '',

            'Statistics Experience'                                => $application->has_experience_formatted,
            'Statistics Experience (Modified)'                     => $applicationVerification->modified->has_experience ?? '',
            'Statistics Experience (Document Provided)'            => $application->has_experience_documents_formatted,
            'Statistics Experience (Document Provided) (Modified)' => $applicationVerification->modified->has_experience_documents ?? '',
            'Statistics Experience Organization'                   => $application->experience_organization,
            'Statistics Experience Organization (Modified)'        => $applicationVerification->modified->experience_organization ?? '',
            'Statistics Experience (Month)'                        => $application->experience_period_month,
            'Statistics Experience (Month) (Modified)'             => $applicationVerification->modified->experience_period_month ?? '',
            'Statistics Experience (Days)'                         => $application->experience_period_day,
            'Statistics Experience (Days) (Modified)'              => $applicationVerification->modified->experience_period_day ?? '',
