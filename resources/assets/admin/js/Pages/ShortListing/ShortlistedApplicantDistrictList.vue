<template>
    <admin-layout :page-title="trans('admin-general.modules.shortlist')" :authorized="isAuthorized($page, 'district_shortlisting')">
        <template #breadcrumb>
            <bread-crumb>
        ...
        <data-table
            ...
            <template #tbody="{row: applicant}" class="text-base-3">
                ...
                <table-body-col>
                    <p>
                        <span v-if="isHired(applicant)" class="text-green-500 cursor-pointer">Hired</span>
                        <span v-else-if="isRejected(applicant)" class="text-danger-500 cursor-pointer">Rejected</span>
                        <span v-else v-text="`Shortlisted`"/>
                    </p>
                </table-body-col>
                <table-body-col>
                    <p>
                        <primary-button @click="showApplicant(applicant)" v-text="`View Application`"/>
                    </p>
                </table-body-col>
            </template>

            <template #empty-message>
                <p>You must select district, local level and role to see the shortlisted applicants.</p>
            </template>
        </data-table>
    </admin-layout>
</template>
...
<script type="text/ecmascript-6">
    ...
    export default {
        name: "ShortListedApplicantDistrictList",
        ...
        methods: {
            loadData(query) {
                ...
                this.$inertia.visit(this.route("admin.shortlisting.district.index"), {
                    method: "get",
                    data: this.query,
                    preserveState: true,
                    preserveScroll: true,
            ...
            showApplicant(applicant) {
                ...
                this.$inertia.visit(this.route("admin.shortlist.district.show", [applicant.id, this.query.role]), {
                    preserveState: true,
                    preserveScroll: true,
                })
            },
            ...
            isShortListed(applicant) {
                return this.getFromObject(applicant, "is_shortlisted", false)
            },
            isHired(applicant) {
                return this.getFromObject(applicant, "hiring_status") === HIRED
            },
            isRejected(applicant) {
                return this.getFromObject(applicant, "hiring_status") === REJECTED
            },
        },
    }
</script>
