<template>
    <admin-layout :authorized="isAuthorized($page, 'daily_report')" :page-title="trans('admin-general.modules.daily-report-list')">
        <template #breadcrumb>
            <bread-crumb>
                <bread-crumb-item :label="trans('admin-general.modules.daily-report-list')"/>
            </bread-crumb>
        </template>
        <template #actions>
            <secondary-button v-if="isAuthorized($page, 'daily_report_create')"
                              :href="route('admin.daily-reports.create')"
                              class="!bg-transparent !py-1 !pl-0 sm:!pl-4 !pr-0">
                <Icon class="!w-5 !h-5 !mr-1" name="plus-1"/>
                {{ trans("admin-general.modules.daily-report-create") }}
            </secondary-button>
        </template>
        ...
        <data-table
            ...
            <template #thead>
                ...
                <table-head-col v-text="`Total Surveyed`"/>
                <table-head-col v-text="`Enumerator`"/>
            ...
            <template #tbody="{row: report}" class="text-base-3">
                ...
                <table-body-col class="text-center" v-text="getFromObject(report, 'total_surveyed')"/>
                <table-body-col v-text="getFromObject(report, 'enumerator')"/>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editReport(report) {
                this.$inertia.visit(this.route("admin.daily-reports.edit", report.id))
            },

            deleteReport(report) {
                this.$inertia.delete(this.route("admin.daily-reports.destroy", report.id))
            },
        },
    }
</script>
